...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if C is not I_4:
            ...
            elif num_channels > 3: ########### Chenaged to apply the color transformation only in the RGB channel for images with alpha blending by Soubhik
                # C = C[:, :3, :].mean(dim=1, keepdims=True)
                # images[:, :3, :] = images[:, :3, :] * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
